...
import { GoogleIconsProvider } from '../../../main/api/googleIconsProvider.js';
...
export class Add extends BinaryOperation {
  ...
  _add() {
    ...
    if (
      leftMatrixInput.rows != rightMatrixInput.rows ||
      leftMatrixInput.columns != rightMatrixInput.columns
    ) {
      alert(
        'При сложении двух матриц число строк и число столбцов каждой матрицы должны быть равными!'
      );
    }
  ...
  _addOperatorIcon() {
    const operator = this.operandsData.operatorButton;
    const icon = GoogleIconsProvider.getIcon('add');
    operator.append(icon);
  }

  _onInput() {
    this._sizeInputs._addOnInputEvent(
      this._inputs.rowsLeft,
      this._inputs.rowsRight
    );
    this._sizeInputs._addOnInputEvent(
      this._inputs.rowsRight,
      this._inputs.rowsLeft
    ...
    this._sizeInputs._addOnInputEvent(
      this._inputs.columnsLeft,
      this._inputs.columnsRight
    );
    this._sizeInputs._addOnInputEvent(
      this._inputs.columnsRight,
      this._inputs.columnsLeft
